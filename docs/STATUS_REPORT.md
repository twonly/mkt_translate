...
- 译文朗读：集成 MiniMax TTS，提供音色选择与音频播放器，未配置凭证时返回本地 Mock 音频。
- 官网翻译工作台：新增 `pages/webpage.html`，支持输入网址抓取官网内容、圈选段落翻译、整页批量翻译与朗读。
...
6. 官网翻译工作台：增强块级识别与元素高亮、支持批量多语言翻译、导出对照版面。
7. MiniMax TTS：缓存策略与批量朗读、音频清理任务。
