...
- **标注提交与列表**
  - 步骤：点击译文中的高亮词，侧栏标注表单自动填充术语信息；选择问题类型、填写描述与建议译法后提交。
  - 预期：接口 `POST /api/annotations` 返回 201；侧栏列表展示新标注并更新计数；历史记录对应条目的 `annotations` 增加，默认状态为“待审核”。
- **标注加载与状态**
  - 步骤：加载已有历史记录，观察侧栏列表；修改标注状态为“已采纳/已驳回”。
  - 预期：`GET /api/annotations?recordId=...` 返回先前标注；侧栏显示列表并可滚动查看；状态切换调用 `PUT /api/annotations/:id` 成功，筛选器“待处理/全部”实时更新计数。
- **备注保存**
