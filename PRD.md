...
- **术语高亮与说明**：命中术语库的词语在译文中高亮，悬浮显示原文字段；自定义术语可叠加并优先。
- **标注面板**：译文右侧提供问题标注与审核流程，支持问题类型、严重级别、建议译法、术语回写等字段；可按状态筛选、切换标注审核结果，悬浮译文高亮即可快速跳转标注。
...
- **朗读播报**：调用 MiniMax TTS 接口为译文生成音频，支持音色选择与播放下载，在无凭证时返回本地 Mock 音频以便演示。
- **官网翻译工作台**：输入官网地址抓取页面内容，左侧显示网页预览并支持圈选段落，右侧沿用翻译/标注/TTS 流程，可翻译单段或整页并记录历史。
